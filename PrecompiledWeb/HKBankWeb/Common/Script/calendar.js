...
var bMoveable=true;
var strFrame;		
document.writeln('<iframe id=endDateLayer frameborder=0 style="position: absolute; width: 162px; height: 211px; z-index: 9998; display: none"></iframe>');
strFrame='<style>';
...
function closeLayer()               //这个层的关闭
  {
	...
	if (o != null)
	{
		...
		DateCtr_OnChange();DateCtr_OnChangeMonth();
	}
  }
...
//日期控件值改变刷新form1
//张虎 2006-8-11
//txtAlterDate保存原来日期控件
//txtDate日期控件
function DateCtr_OnChange()
{
   var olddate = document.getElementById('txtAlterDate');
   var date = document.getElementById('txtDate');
   if(date != null && olddate != null && olddate.value != date.value )
   {
	   document.form1.submit();
   }
}
function DateCtr_OnChangeMonth()
{
    var olddate = document.getElementById('txtAlterDate1');
    var date = document.getElementById('txtDate1');
    if(date != null && olddate != null)
    {
        var oldValueArr = olddate.value.split('-');
        var valueArr = date.value.split('-');
        if (oldValueArr[0] != valueArr[0] || (oldValueArr[0] == valueArr[0] && oldValueArr[1] != valueArr[1]))
            document.form1.submit();
    }
}
